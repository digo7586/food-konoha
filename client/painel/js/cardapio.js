...
cardapio.method = {
    ...
    obterProdutosCategoria: (idcategoria, forcar = false) => {
        ...
        if (conteudo <= 0 || forcar) {
            ...
            let listaProdutosDiv = document.getElementById("listaProdutos-" + idcategoria);
            if (listaProdutosDiv) {
...
const precoFmt = Number.isFinite(precoNum) ? precoNum.toFixed(2).replace('.', ',') : '0,00';
    ...
    editarProduto: (idcategoria, idproduto) => {
        ...
        if (produto.length > 0) {
            ...
            $("#txtPrecoProduto").val(Number(produto[0].valor).toFixed(2).replace('.', ','));
            ...
            Object.entries(listaAgrupada).forEach((e, i) => {
                ...
                for (let index = 0; index < opcional.length; index++) {
                    ...
                    if (element.valoropcional > 0) {
                        valor = `+ R$ ${Number(element.valoropcional).toFixed(2).replace('.', ',')}`;
                    }
                ...
                let listaDiv = document.querySelector("#listaOpcionaisProduto");
                if (listaDiv) {
                    listaDiv.innerHTML += temp;
                }
        ...
        if (listaSimples.length > 0) {
            ...
            listaSimples.forEach((e, i) => {
                ...
                if (e.valoropcional > 0) {
                    valor = `+ R$ ${Number(e.valoropcional).toFixed(2).replace('.', ',')}`;
                }
    ...
    atualizarHintOpcao: () => {
        ...
        if (maximo === 0 && minimo === 0) {
            texto = 'Nenhuma opção selecionada';
        }
...
cardapio.template = {
    ...
    <div class="card mt-3" data-idcategoria="\${id}">
      <div class="card-drag" id="heading-\${id}">
        ...
        <div class="infos">
          <a href="#!" class="name mb-0"
             ...
             onclick="CATEGORIA_ID='\${id}'; cardapio.method.obterProdutosCategoria('\${id}')">
      ...
      <div id="collapse-\${id}" class="collapse" aria-labelledby="heading-\${id}" data-parent="#categoriasMenu">
        <div class="card-body">
          ...
          <div class="lista-produtos" id="listaProdutos-\${id}"></div>
  ...
  opcionalSimples: `
    <section class="opcionais-simples">
      ...
      <div class="container-group mb-5" id="listaOpcionaisSimples"></div>
    </section>
  `,
