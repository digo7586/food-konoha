...
relatorio.method = {
    ...
    atualizarTotais: (lista) => {
        ...
        if (lista.length > 0) {
            ...
            $.each(lista, (i, e) => {
                let valorTotal = Number(e.total) || 0;
                let pedidos = Number(e.pedidos) || 0;

                totalFaturado += valorTotal;
                totalPedidos += pedidos;
            })

            let ticketMedio = totalPedidos > 0 ? (totalFaturado / totalPedidos) : 0;

            $("#lblTotalFaturamento").text(`R$ ${totalFaturado.toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`);

            $('#lblTotalPedidos').text(totalPedidos);
            $("#lblTicketMedio").text(`R$ ${ticketMedio.toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`);
    ...
    listarPedidos: (list) => {
        ...
        if (list.length > 0) {
            ...
            $.each(list, (i, e) => {
               ...
               let colorTotal = (e.idpedidostatus == 6) ? '#e53935' : '#179e40';
let valorFormatado = `R$ ${(parseFloat(e.total).toFixed(2)).toString().replace('.', ',')}`;
...
let _temp = relatorio.template.trPedido.replace(/\${idpedido}/g, e.idpedido)
    ...
    .replace('style="color: #179e40;"', `style="color: ${colorTotal};"`)
    .replace(/\${total}/g, valorFormatado);
