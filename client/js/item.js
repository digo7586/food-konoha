    ...
    obterDadosProduto: () => {
        ...
        app.method.get('/produto/' + ITEM_ID,
            (response) => {
                ...
                document.getElementById("preco-produto").innerText = "R$ " + Number(produto.valor).toFixed(2).replace('.', ',');
                document.getElementById("btn-preco-produto").innerText = `R$ ${(Number(produto.valor)).toFixed(2).replace('.', ',')}`;
        ...
        if (lista.length > 0) {
            ...
            Object.entries(listaAgrupada).forEach((e, i) => {
                ...
                for (let index = 0; index < opcional.length; index++) {
                    ...
                    if (Number(element.valoropcional) > 0) {
