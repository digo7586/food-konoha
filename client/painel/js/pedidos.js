...
pedido.method = {
    ...
    carregarPedidos: (lista) => {
        ...
        function toMoneyBR(value) {
            if (value === null || value === undefined || value === '') return null;
            const num = Number(String(value).replace(',', '.'));
            return Number.isFinite(num) ? num.toFixed(2).replace('.', ',') : null;
        }
        ...
        if (lista.length > 0) {
            ...
            lista.forEach((e, i) => {
                ...
                const totalFmt = toMoneyBR(e.total) || '0,00';
                const trocoFmt = toMoneyBR(e.troco);
                ...
                if (e.idformapagamento == 1) {
                    ...
                    formapagamentodesc = trocoFmt
                        ? `Troco para: R$ ${trocoFmt} reais`
    ...
    carregarTotais: (data) => {
        ...
        document.querySelector("#badge-total-cancelado").classList.add('hidden');
        ...
        if (data.cancelado > 0) {
            document.querySelector("#badge-total-cancelado").classList.remove('hidden');
            document.querySelector("#badge-total-cancelado").innerText = data.cancelado;
        }
    ...
    carregarModalDetalhes: (data, idpedido, cart) => {
        ...
        if (data.idformapagamento == 1) {
            ...
            document.querySelector("#lblFormaPagamentoDescricao").innerText = Number(data.troco) > 0
                ? `Troco para: ${Number(data.troco).toFixed(2).replace('.', ',')} reais`
                : 'Pagamento na entrega do pedido';
        }
        else {
       ...
       else if (data.idpedidostatus == 6) {
                document.querySelector("#container-action-footer").innerHTML = `<button onclick="pedido.method.moverPara(1, '${idpedido}')" type="button" class="btn btn-yellow btn-sm">Recuperar Pedido</button>`;
             }
        else {
        ...
        order.forEach((e, i) => {
            ...
            if (e.opcionais.length > 0) {
                ...
                for (let index = 0; index < e.opcionais.length; index++) {
                    ...
                    itens += pedido.template.opcional.replace(/\${nome}/g, `${e.quantidade}x ${element.nomeopcional}`)
                        .replace(/\${preco}/g, `+ R$ ${(Number(element.valoropcional) * Number(e.quantidade)).toFixed(2).replace('.', ',')}`)
                }
            }
            ...
            let temp = pedido.template.produto.replace(/\${guid}/g, e.guid)
                ...
                .replace(/\${preco}/g, `R$ ${(Number(e.quantidade) * Number(e.valor)).toFixed(2).replace('.', ',')}`)
        ...
        if (data.taxaentrega > 0) {
            let temptaxa = pedido.template.taxaentrega.replace(/\${total}/g, `+ R$ ${Number(data.taxaentrega).toFixed(2).replace('.', ',')}`)
            document.querySelector("#itensPedido").innerHTML += temptaxa;
        }

        let temptotal = pedido.template.total.replace(/\${total}/g, `R$ ${Number(data.total).toFixed(2).replace('.', ',')}`)
    ...
    atualizarLista: () => {
        ...
        else if (tabAtiva == 'tab-concluido') {
            pedido.method.openTab('concluido', 5);
        }
        else if (tabAtiva == 'tab-cancelado') {
            pedido.method.openTab('cancelado', 6);
        }
