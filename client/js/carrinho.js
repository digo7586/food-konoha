        ...
        if (CARRINHO_ATUAL.length > 0) {
            ...
            if (Number(TAXA_ATUAL) > 0) {
                ...
                document.querySelector("#lblTaxaEntrega").innerText =
                    `+ R$ ${Number(TAXA_ATUAL).toFixed(2).replace('.', ',')}`;
            }
            else {
            ...
            document.querySelector("#lblTotalCarrinho").innerText =
                `R$ ${Number(total).toFixed(2).replace('.', ',')}`;
            document.querySelector("#lblTotalCarrinhoBotao").innerText =
                `R$ ${Number(total).toFixed(2).replace('.', ',')}`;
        }
    ...
    validarEnderecoSelecionado: () => {
        ...
        if (TAXAS_ENTREGA[0].idtaxaentregatipo == 2) {
            ...
            if (enderecoAtual != undefined) {
                ...
                console.log("Endereço do cliente para cálculo de taxa:", dados);
