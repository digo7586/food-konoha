            ...
            app.method.get('/pedido/' + order.order,
                (response) => {
                    ...
                    let temp = pedido.template.dadospedido.replace(/\${data}/g, `${dataFormatada} às ${horarioFormatado}`)
                        .replace(/\${valor}/g, `R$ ${Number(response.data.total).toFixed(2).replace('.', ',')}`);
        ...
        ORDER.cart.forEach((e, i) => {
            ...
            if (e.opcionais.length > 0) {
                ...
                for (let index = 0; index < e.opcionais.length; index++) {
                    ...
                    subTotal += Number(element.valoropcional) * Number(e.quantidade);
                }
            }

            subTotal += Number(e.quantidade) * Number(e.valor);
            total += subTotal;

        });

        /* console.log("typeof total:", typeof total, "typeof taxa:", typeof ORDER.taxaentrega);
console.log("total antes:", total, "taxa:", ORDER.taxaentrega); */


        // valida se tem taxa 
        if (ORDER.taxaentrega > 0) {
            total = Number(total) + Number(ORDER.taxaentrega);

           let temptaxa = pedido.template.taxaentrega.replace(
    /\${total}/g,
    `+ R$ ${Number(ORDER.taxaentrega).toFixed(2).replace('.', ',')}`
       ...
       let temptotal = pedido.template.total.replace(/\${total}/g, `R$ ${Number(total).toFixed(2).replace('.', ',')}`)
    ...
    mensagemWhatsApp: () => {
        ...
        let URL = `https://wa.me/5511962795416?text=${encode}`;
